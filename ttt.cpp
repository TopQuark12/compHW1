/*
 * skeleton.cpp
 * For lab4     Super Tic-Tac-Toe
 *
 * This is a skeleton code for students who need hints to finish the lab
 * The super Tic-Tac-Toe can work when you implements the FUNCTIONs
 * But feel free to change code elsewhere if you want
 * 
 */
...
bool checkWin(int player, const int board[][3])
{
    ...
    for (int x = 0; x < 3; x++) {
        if ((board[x][0] == player && board[x][1] == player && board[x][2] == player) || //row equal
            (board[0][x] == player && board[1][x] == player && board[2][x] == player) || //column equal
            (board[0][0] == player && board[1][1] == player && board[2][2] == player) || // \equal
            (board[0][2] == player && board[1][1] == player && board[2][0] == player))   // /equal
        {
            return true;
        }
    }
...
int main()
{
    ...
    int board[9][9] = {};           //array storing information of the whole playing field
    int superBoard[3][3] = {};      //array abstracting 9x9 board details into 3x3 superboard regions
    int tempBoard[3][3] = {};       //array storing a single superboard region of interst
